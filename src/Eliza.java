...
public class Eliza {
	 ...
	 * from Config.POST_PROCESS_AFTER. (Update 2/17) If draftResponse is null,
	...
	public static String matchResponse(String[] words) {
		...
		if (words[0] == null) {
			...
			= Eliza.chooseString(Config.NO_MATCH_RESPONSES);
		...
		for (int i = 0; i < Config.RESPONSE_TABLE.length; i++) {
			...
			if (i == Config.RESPONSE_TABLE.length - 1) {
				String randomResponse 
				= Eliza.chooseString(Config.NO_MATCH_RESPONSES);
				return randomResponse;				
			}
	...
	public static String processInput(String userInput) {
		...
		if (Eliza.findAnyWords(split, Config.QUIT_WORDS)) {
			return null;
		}

		// Replace any words in Config.PRE_PROCESS_BEFORE with corresponding
		// words in Config.PRE_PROCESS_AFTER
